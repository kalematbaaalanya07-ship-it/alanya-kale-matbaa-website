...
export function HolidayCalendar({ special, publicHolidays, summary }: { special: SpecialDay[]; publicHolidays: PublicHoliday[]; summary: Summary[] }) {
  ...
  return (
    <>
      ...
      <section className="mt-8 rounded-2xl border border-border bg-primary p-5 text-primary-foreground sm:p-6"><h2 className="font-heading text-xl font-bold">Resmi Tatiller</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{publicHolidays.map((item) => <div key={`${item.date}-${item.name}`} className="rounded-xl bg-white/10 p-4"><p className="font-semibold">{item.name}</p><p className="mt-1 text-sm text-primary-foreground/75">{item.date} · {item.day}</p></div>)}</div></section>
